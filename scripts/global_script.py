...
json_decode = json.load(open(path + "//Inputs-Global//global.json", encoding='utf-8'))
...
for i in range(0, len(json_decode)):
    ...
    for item in val:
        my_dict={}
        if not str(item['value']).endswith('.svg') and str(item['value']) != 'False' and str(item['value']) != 'True' and not str(item['value']).isnumeric():
            my_dict['SourceText']=item['value']
            my_dict['text']=item['value']
            my_dict['type']='global'
            if str(item['value']).count('@') == 1:
                for tx in lst:
                    if str(tx) in str(item['value']):
                        my_dict['note']='The string '+ tx +' should not be translated.'
            elif str(item['value']).count('@') > 1:
                st = []
            #print(str(val['SourceText']))
                for tx in lst:
                    if str(tx) in str(item['value']):
                        st.append(tx)
                if len(st) == 1:
                    my_dict['note']='The string '+ st[0] +' should not be translated.'
                if len(st) == 2:
                    my_dict['note']='The following strings should not be translated: ' + st[0] + ' ' + st[1]
                if len(st) == 3:
                    my_dict['note']='The following strings should not be translated: ' + st[0] + ' ' + st[1] + ' ' + st[2]
                if len(st) == 4:
                    my_dict['note']='The following strings should not be translated: ' + st[0] + ' ' + st[1] + ' ' + st[2] + ' ' + st[3]
                if len(st) == 5:
                    my_dict['note']='The following strings should not be translated: ' + st[0] + ' ' + st[1] + ' ' + st[2] + ' ' + st[3] + ' ' + st[4]
...
with open(path + "//Outputs-Global//global.json", 'w', encoding='utf-8') as json_file:
