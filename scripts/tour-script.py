...
json_decode = json.load(open(path + "//Inputs-Tour//tour.json", encoding='utf-8'))
...
for i in range(0, len(json_decode)):
    ...
    for item in val:
        my_dict={}
        if not str(item.get('title')) in lst and not str(item.get('message_text')) in lst:
            if 'title' in str(item):
                my_dict['SourceText1']=item['title']
                my_dict['text1']=item['title']
                if str(item['title']).count('@') == 1:
                    for tx in lst:
                        if str(tx) in str(item['title']):
                            my_dict['note1']='The string '+ tx +' should not be translated.'
                if str(item['title']).count('@') > 1:
                    st = []
                    for tx in lst:
                        if str(tx) in str(item['title']):
                            st.append(tx)
                    if len(st) == 1:
                        my_dict['note1']='The string '+ st[0] +' should not be translated.'
                    if len(st) == 2:
                        my_dict['note1']='The following strings should not be translated: ' + st[0] + ' ' + st[1]
                    if len(st) == 3:
                        my_dict['note1']='The following strings should not be translated: ' + st[0] + ' ' + st[1] + ' ' + st[2]
                    if len(st) == 4:
                        my_dict['note1']='The following strings should not be translated: ' + st[0] + ' ' + st[1] + ' ' + st[2] + ' ' + st[3]
                    if len(st) == 5:
                        my_dict['note1']='The following strings should not be translated: ' + st[0] + ' ' + st[1] + ' ' + st[2] + ' ' + st[3] + ' ' + st[4]

            my_dict['SourceText2']=item['message_text']
            my_dict['text2']=item['message_text']
            my_dict['type']='tour'
            if str(item['message_text']).count('@') == 1:
                my_dict['note2']='The string '+ tx +' should not be translated.'
            if str(item['message_text']).count('@') > 1:
                st = []
                for tx in lst:
                    if str(tx) in str(item['message_text']):
                        st.append(tx)
                if len(st) == 1:
                    my_dict['note2']='The string '+ st[0] +' should not be translated.'
                if len(st) == 2:
                    my_dict['note2']='The following strings should not be translated: ' + st[0] + ' ' + st[1]
                if len(st) == 3:
                    my_dict['note2']='The following strings should not be translated: ' + st[0] + ' ' + st[1] + ' ' + st[2]
                if len(st) == 4:
                    my_dict['note2']='The following strings should not be translated: ' + st[0] + ' ' + st[1] + ' ' + st[2] + ' ' + st[3]
                if len(st) == 5:
                    my_dict['note2']='The following strings should not be translated: ' + st[0] + ' ' + st[1] + ' ' + st[2] + ' ' + st[3] + ' ' + st[4]
...
with open(path + "//Outputs-Tour//tour.json", 'w', encoding='utf-8') as json_file:
